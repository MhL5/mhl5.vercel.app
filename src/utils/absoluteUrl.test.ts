...
describe("absoluteUrl", () => {
  describe("development mode", () => {
    ...
    test("should generate correct URL for simple path", () => {
      ...
      expect(result).toBe("http://localhost:7777/blogs");
    });
    ...
    test("should normalize duplicate slashes", () => {
      ...
      expect(result).toBe("http://localhost:7777/path/to/resource");
    });
  ...
  describe("edge cases", () => {
    test("should handle paths with encoded characters", () => {
      ...
      expect(result).toBe("http://localhost:7777/path%20with%20spaces");
    });
